...
- v1で未定義の欠落割合をfinding件数から作らない。partial_warning_thresholdは予約値として検証し、完全性を変更しない。baselineの明示参照を別ファイルへ置き換えない。
- policyの既知項目は解析済みYAMLから読む。真偽値の末尾コメントで判定を変えず、文字列のコロン・空白・引用符の解釈をYAMLライブラリへ委ねる。同梱policyは実loaderで意図した設定値を確認する。
...
- 実行識別：独立実行のrun IDと内容fingerprintを区別し、agentの冪等再利用を維持する。読込済みartifactのIDを書き換えない。
- 差分解析の完全性：指摘件数や意図した対象範囲の絞り込みからpartialを判定しない。走査上限・読み取り失敗などの実際の欠落を保持し、指摘が見つかったことだけでcompleteへ昇格させない。
- readiness入力：JSONの型キャストを検証の代わりにせず、評価前に既存schemaへ適合するfindingsであることを検証する。
- diffの参照：指定headを固定した同一snapshotから本文・hash・行数・影響範囲を作り、未コミットの作業内容を混入させない。取得失敗や上限超過はpartialとして保持する。
