...
Market traction metrics are tracked separately and are not product quality gates:

| metric | target | use |
|---|---:|---|
| GitHub stars | 100+ | Launch / community signal |
| Real project usage | 10+ references | Adoption signal, not analyzer correctness evidence |

---

## 8. 段階定義

### 8.0 Current Maturity Alignment (2026-07-04)

Phase 1-3 の機能実装は多くが完了済みだが、product maturity 判定は
`docs/product-maturity-issues.md` と `docs/acceptance-evidence-index.md` の
証跡に従う。特に public release / v1.0 quality claim は、real public repo
precision、distribution evidence、package publication evidence が揃うまで
beta相当の判断を維持する。
...
| method | 内容 |
|---|---|
| npm | Future npm path: `npm install -g @quality-harness/code-to-gate`; use `npm install -g github:RNA4219/code-to-gate` until publication is complete |
| Binary | Future scope; no pre-built Windows/macOS/Linux binary is distributed in the current package |
| Docker | Future CLI distribution option; current Docker image support is limited to plugin sandbox execution |
...
|---|:---:|---|
| Gatefield | Phase 1+ | `ctg.gatefield/v1` (`v1alpha1` legacy-compatible) |
| State Gate | Phase 1+ | `ctg.state-gate/v1` (`v1alpha1` legacy-compatible) |
| Manual-bb | Phase 1+ | `ctg.manual-bb/v1` (`v1alpha1` legacy-compatible) |
| Workflow evidence | Phase 1+ | `ctg.workflow-evidence/v1` (`v1alpha1` legacy-compatible) |
...
| Market traction | 100+ GitHub stars tracked separately from quality GO/NO-GO |
