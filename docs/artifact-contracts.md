...
- `pr-review.json` が正本で、`pr-review.md` は同じ内容をPRコメント用に整形した派生物。
- LLM は不要。block理由、許容理由、追加テスト、仕様差分、artifact link は既存artifactから deterministic に生成する。
- GitHubへの投稿はこのartifactの外側で行い、CIは `status: "block"` を `READINESS_NOT_CLEAR` として扱える。

### 10.1 GateExplainability

`gate-explainability@v1` は `release-readiness.json` と `findings.json` から
deterministic に生成する gate failure 説明 artifact である。LLM 判断は使わず、
failed condition、blocking finding、manual evidence 候補、baseline 更新候補、
severity 再評価候補を列挙する。

```ts
interface GateExplainabilityArtifact extends ArtifactHeader {
  ...
  sourceArtifacts: { file: string; schema?: string; hashSha256: string }[];
  generated_by: "ctg-gate-explainability-v1";
}
```

PR review と release-pack はこの artifact が存在する場合、required action 数を
人間向け summary に表示する。

---

## 10.2 RedactionProfile

`redaction-profile@v1` は artifact と human surface の情報量を切り替える
profile contract である。profile は `public`、`private`、`regulated` を持つ。

```ts
type RedactionProfileName = "public" | "private" | "regulated";
...
- `public` は path/hash/count 中心で、excerpt/detail は redaction 対象にする。
- `private` は excerpt/detail を許可する。
- `regulated` は excerpt/detail を許可するが、signer、retention、
  approval binding が無い場合は `warnings` に記録する。
- viewer、release-pack、PR review、query artifact は profile 指定時に
  `redactionProfile` と `redactionSummary` を記録する。

---

## 10.3 RuleQualityScore

`rule-quality-score@v1` は rule/plugin 単位で fixture coverage、false-positive
review、evidence completeness、schema compatibility、runtime cost を採点する
artifact である。score は算出式と入力 evidence を保持し、コミュニティレビューで
再計算できる形にする。

```ts
interface RuleQualityScoreArtifact extends ArtifactHeader {
...
`plugin-marketplace@v1` は plugin directory に `rule-quality-score.json` が存在する場合、
entry の `qualityScore` に total score、grade、各metric score、score artifact path を
接続する。
...
## 13. HostedEvidencePortal

`hosted-evidence-portal@v1` は複数 run の evidence を静的HTMLで横断する
ための manifest。外部 network を要求せず、run index、artifact hash、
search index、public URL、redaction profile を保持する。
...
## 14. BaselineDebtLedger

`baseline-debt-ledger@v1` は `release-readiness.json.baseline` と後方互換に
接続し、baseline debt の owner、expiry、承認者、承認理由、更新理由、
残工数、再発防止メモを保持する。
...
Expired item は `review-queue.json` の `baseline_expiry` item として接続でき、
`release-pack --include-optional` と hosted evidence portal の入力にできる。
...
- `schemas/redaction-profile.schema.json`
...
- `schemas/baseline-debt-ledger.schema.json`
- `schemas/hosted-evidence-portal.schema.json`
