...
| `--emit` | string | `all` | 出力形式 (`all`, `json`, `yaml`, `md`, `mermaid`, `sarif`) |
| `--policy` | string | - | Policy file path |
| `--llm-provider` | string | `deterministic` | LLM provider (`deterministic`, `ollama`, `llamacpp`, `openai` など) |
| `--llm-mode` | string | `local-only` | LLM policy mode (`local-only`, `allow-cloud`)。provider 選択は `--llm-provider` が担う |
...
**Usage**:
```
code-to-gate scan <repo-path> --out <dir> [--database-analysis]
```
...
| option | type | default | 内容 |
|---|---|---|---|
| `--database-analysis` | boolean | false | SQL and migration risk analysis を有効化 |
| `--cache` | string | `enabled` | `enabled`, `disabled`, `force` |
| `--parallel` | number | `4` | Parser worker 数 |

Note: 現行 CLI には `--languages` / `--exclude` はない。言語判定と標準除外は scanner の既定実装に従う。
...
**目的**: Full analysis artifact generation (scan + rules + optional LLM). Release readiness evaluation is a separate `readiness` command.
...
| Invariants | `.qh/invariants.json` | InvariantsArtifact | ✅ implemented (v1.1) |
| Test seeds | `.qh/test-seeds.json` | TestSeedsArtifact | ✅ implemented (v1.1) |
| Analysis report | `.qh/analysis-report.md` | Human-readable report | ✅ implemented |
| Audit | `.qh/audit.json` | AuditArtifact | ✅ implemented |

**Note**: `test-seeds.json` and `invariants.json` are generated by `--emit all` since v1.1. `release-readiness.json` is generated by `readiness --from <artifact-dir>`.

**Exit codes**:
| code | condition |
|---:|---|
| 0 | analysis artifacts generated successfully |
| 1 | policy-backed analysis reports `needs_review` or `blocked_input` |
...
| `--from` | Yes | Existing artifact directory from `analyze` |
...
| command | 内容 |
|---|---|
| `status` | Docker / sandbox availability check |
| `run <plugin-path> --input <file>` | Execute plugin in sandbox mode |
| `build-image` | Build Docker image for plugin execution |
...
### 3.10 Fixture verification

現行 CLI に `fixture run` サブコマンドはない。Fixture 検証は `fixtures/<name>` を通常 repo として `scan` / `analyze` / `readiness --from` に渡して実行する。
...
analyze
  ...
  ├─ invariants.json (InvariantsArtifact)
...
- Type inference tracking ✅ (`src/adapters/ts-adapter.ts` typeInfo)
...
### 17.2 Report Profiles

Human-facing reports MUST distinguish machine gate severity from review wording.
The default artifact schema keeps full fidelity for downstream consumers, while
report profiles control presentation only.

| profile | audience | behavior |
|---|---|---|
| `machine` | CI, downstream gates, QEG/manual-bb adapters | Preserve current severity/category/status labels and complete evidence payloads. |
| `human` | PR reviewers, QA, tech leads | Present findings as review-required candidates with impact hypothesis, evidence, confidence, and suggested verification steps. Avoid wording that implies confirmed vulnerability. |
| `qa-chain` | HATE/manual-bb/QEG handoff | Preserve structured seeds, known gaps, oracle gaps, and review hints without reducing evidence detail. |

Presentation rules:
- `critical` and `high` are gate severity labels, not proof of confirmed incidents.
- Human reports should show `confidence`, `evidenceKind`, `reviewHint`, and affected artifact paths when available.
- `release-readiness` output is release review input. It does not replace human or downstream approval gates.
...
|---|---|---|
| `ctg/v1` | Stable schema for current examples and generated artifacts | Not allowed without a new major schema |
| `ctg/v1alpha1` | Backward-compatible legacy input | Accepted for migration, not preferred for new examples |
| artifact-specific preview schemas | Experimental surfaces such as database analysis | Allowed until promoted to stable |
...
|---|---|---|
| Gatefield | `ctg.gatefield/v1` | `ctg.gatefield/v1alpha1` is legacy-compatible |
| State Gate | `ctg.state-gate/v1` | `ctg.state-gate/v1alpha1` is legacy-compatible |
| Manual-bb | `ctg.manual-bb/v1` | `ctg.manual-bb/v1alpha1` is legacy-compatible |
| Workflow evidence | `ctg.workflow-evidence/v1` | `ctg.workflow-evidence/v1alpha1` is legacy-compatible |
...
| migration | 内容 |
|---|---|
| CLI options | Keep current `--llm-mode`; `--languages` / `--exclude` require implementation before docs advertise them |
...
**Completed**: SN-01~06
**Remaining**: none for this Next Actions table. New follow-up work is tracked
in `docs/acceptance-evidence-index.md`, `docs/product-maturity-issues.md`, and
