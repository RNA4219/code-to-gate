...
- `package.json` の version、GitHub Release、npm registry の公開 version が一致するか確認する。一致しない場合は `docs/distribution-status.md` と README に差分を明記する。
- `npm view @quality-harness/code-to-gate version dist-tags --json` の結果を記録する。`E404` の場合は npm 未公開 blocker として扱い、GitHub/source install 前提を維持する。
- `npm pack --dry-run` で配布物内容を確認する。
- `npm pack --dry-run` の出力に `dist/cli.js`、`dist/cli.d.ts`、`schemas/`、`LICENSE`、README 群、`CHANGELOG.md` が含まれることを確認する。
...
| npm publication | `npm view @quality-harness/code-to-gate version dist-tags --json` |
