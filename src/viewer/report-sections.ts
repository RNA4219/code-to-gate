...
/**
 * Resolve the producer version from whichever artifact backs this report.
 * Findings are preferred when present, but graph/readiness-only reports must
 * retain the version carried by their own artifact as well.
 */
export function resolveArtifactToolVersion(artifacts: LoadedArtifacts): string | undefined {
  return artifacts.findings?.tool?.version
    ?? artifacts.riskRegister?.tool?.version
    ?? artifacts.testSeeds?.tool?.version
    ?? artifacts.readiness?.tool?.version
    ?? artifacts.graph?.tool?.version
    ?? artifacts.historicalComparison?.tool?.version
    ?? artifacts.evidenceDag?.tool?.version;
}
...
export function generateHeader(
  ...
  const toolVersion = resolveArtifactToolVersion(artifacts) ?? UNKNOWN_VERSION;
 ...
 */
export function generateFooter(toolVersion = UNKNOWN_VERSION): string {
  return `
<div class="footer">
  <p>Generated by code-to-gate v${escapeHtml(toolVersion)}</p>
